...
public class ElasticSearchUtilsIT {
	...
	public void testCreateDeleteExistsIndex() throws InterruptedException {
		...
		assertTrue(ElasticSearchUtils.existsIndex(appName1));
		
		assertTrue(ElasticSearchUtils.optimizeIndex(appName1));
		
		ElasticSearchUtils.deleteIndex(appName1);
		assertFalse(ElasticSearchUtils.existsIndex(appName1));
		
		assertFalse(ElasticSearchUtils.createIndex(badAppName));
		assertFalse(ElasticSearchUtils.existsIndex(badAppName));
		assertFalse(ElasticSearchUtils.deleteIndex(appName1));
	}
